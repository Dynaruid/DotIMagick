...
public sealed class Pixel : IPixel<QuantumType>
{
    ...
    /// </summary>
    public int Channels => _value.Length;
    ...
    public QuantumType[] ToArray() => _value;
    ...
    public IMagickColor<QuantumType>? ToColor()
    {
        ...
        var hasBlackChannel =
            _collection is not null && _collection.GetIndex(PixelChannel.Black) != -1;
        var hasAlphaChannel =
            _collection is not null && _collection.GetIndex(PixelChannel.Alpha) != -1;
    ...
    private static void CheckChannels(int channels) =>
        Throw.IfTrue(
            nameof(channels),
            channels < 1 || channels > 5,
            "Invalid number of channels (supported sizes are 1-5)."
        );
