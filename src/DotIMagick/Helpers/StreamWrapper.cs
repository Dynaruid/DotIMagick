...
internal sealed unsafe class StreamWrapper : IDisposable
{
    ...
    public long Seek(long offset, IntPtr whence, IntPtr user_data)
    {
        try
        {
            ...
            {
                SeekOrigin.Begin
                    => _stream.Seek(_streamStart + offset, SeekOrigin.Begin) - _streamStart,
    ...
    {
        Helpers.NativeMemory.Copy(source, _bufferStart, length);
        return source + length;
    }

    private byte* ReadBuffer(byte* destination, long length)
    {
        Helpers.NativeMemory.Copy(_bufferStart, destination, length);
