...
public abstract partial class DoubleMatrix
{
    ...
    protected DoubleMatrix(int order, double[]? values)
    {
        Throw.IfTrue(
            nameof(order),
            order < 1,
            "Invalid order specified, value has to be at least 1."
        );
    ...
    public double GetValue(int x, int y) => _values[GetIndex(x, y)];
    ...
    public void SetValue(int x, int y, double value) => _values[GetIndex(x, y)] = value;
