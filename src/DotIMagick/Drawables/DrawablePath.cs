...
public sealed class DrawablePath : IDrawablePath, IDrawingWand
{
    ...
    public DrawablePath(params IPath[] paths) => _paths = new List<IPath>(paths);

    /// <summary>
    /// Initializes a new instance of the <see cref="DrawablePath"/> class.
    /// </summary>
    /// <param name="paths">The paths to use.</param>
    public DrawablePath(IEnumerable<IPath> paths) => _paths = new List<IPath>(paths);

    /// <summary>
    /// Gets the paths to use.
    /// </summary>
    public IReadOnlyCollection<IPath> Paths => _paths;
