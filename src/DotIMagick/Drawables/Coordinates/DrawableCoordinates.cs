...
internal abstract class DrawableCoordinates<TCoordinateType> : IReadOnlyList<TCoordinateType>
{
    ...
    protected DrawableCoordinates(IEnumerable<TCoordinateType> coordinates, int minCount)
    {
        ...
        _coordinates = DrawableCoordinates<TCoordinateType>.CheckCoordinates(
            new List<TCoordinateType>(coordinates),
            minCount
        );
    }
    ...
    public TCoordinateType this[int index] => _coordinates[index];

    public IEnumerator<TCoordinateType> GetEnumerator() => _coordinates.GetEnumerator();

    IEnumerator IEnumerable.GetEnumerator() => _coordinates.GetEnumerator();

    private static List<TCoordinateType> CheckCoordinates(
        List<TCoordinateType> coordinates,
        int minCount
    )
    {
        ...
        foreach (var coordinate in coordinates)
        {
            if (coordinate is null)
                throw new ArgumentNullException(
                    nameof(coordinates),
                    "Value should not contain null values"
                );
        }

        if (coordinates.Count < minCount)
            throw new ArgumentException(
                "Value should contain at least " + minCount + " coordinates.",
                nameof(coordinates)
            );
