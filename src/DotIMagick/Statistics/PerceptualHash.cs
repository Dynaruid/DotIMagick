...
public sealed partial class PerceptualHash : IPerceptualHash
{
    ...
    public PerceptualHash(string hash, params ColorSpace[] colorSpaces)
    {
        ...
        _channels[PixelChannel.Red] = new ChannelPerceptualHash(
            PixelChannel.Red,
            colorSpaces,
            hash.Substring(0, length)
        );
        _channels[PixelChannel.Green] = new ChannelPerceptualHash(
            PixelChannel.Green,
            colorSpaces,
            hash.Substring(length, length)
        );
        _channels[PixelChannel.Blue] = new ChannelPerceptualHash(
            PixelChannel.Blue,
            colorSpaces,
            hash.Substring(length + length, length)
        );
    }
    ...
    internal static ColorSpace[] DefaultColorSpaces { get; } =
        new[] { ColorSpace.sRGB, ColorSpace.HCLp };
    ...
    public double SumSquaredDistance(IPerceptualHash other)
    {
        ...
        return _channels[PixelChannel.Red].SumSquaredDistance(other.GetChannel(PixelChannel.Red))
    ...
    internal static void ValidateColorSpaces(ColorSpace[] colorSpaces)
    {
        ...
        Throw.IfFalse(
            nameof(colorSpaces),
            colorSpaces.Distinct().Count() == colorSpaces.Length,
    ...
    private static ChannelPerceptualHash? CreateChannelPerceptualHash(
        IMagickImage image,
