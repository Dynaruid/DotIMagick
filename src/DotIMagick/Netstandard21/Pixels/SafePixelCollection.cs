...
internal sealed partial class SafePixelCollection
{
    ...
    public override void SetArea(
        ...
        int width,
        int height,
        ReadOnlySpan<QuantumType> values
    )
    {
    ...
    private void CheckValues<T>(ReadOnlySpan<T> values) => CheckValues(0, 0, values);
    ...
    private void CheckValues<T>(int x, int y, int width, int height, ReadOnlySpan<T> values)
    {
        ...
        Throw.IfFalse(
            nameof(values),
            values.Length % Channels == 0,
            "Values should have {0} channels.",
            Channels
        );
