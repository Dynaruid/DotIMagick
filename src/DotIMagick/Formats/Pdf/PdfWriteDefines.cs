...
public sealed class PdfWriteDefines : IWriteDefines
{
    ...
    /// </summary>
    public MagickFormat Format => MagickFormat.Pdf;
    ...
    public IEnumerable<IDefine> Defines
    {
        get
        {
            ...
            if (CreationTime is not null)
                yield return new MagickDefine(
                    Format,
                    "create-epoch",
                    ToUnixTimeSeconds(CreationTime.Value)
                );
            ...
            if (ModificationTime is not null)
                yield return new MagickDefine(
                    Format,
                    "modify-epoch",
                    ToUnixTimeSeconds(ModificationTime.Value)
                );
            ...
            if (Version.HasValue)
                yield return new MagickDefine(
                    Format,
                    "version",
                    Version.Value.ToString(".0", CultureInfo.InvariantCulture)
                );
