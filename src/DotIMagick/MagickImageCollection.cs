...
public sealed partial class MagickImageCollection : IMagickImageCollection<QuantumType>
{
    ...
    public void AddRange(IEnumerable<IMagickImage<QuantumType>> images)
    {
        ...
        Throw.IfTrue(
            nameof(images),
            images is MagickImageCollection,
            "Not allowed to add collection."
        );
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickImage<QuantumType> Combine() => Combine(ColorSpace.sRGB);
    ...
    public bool Contains(IMagickImage<QuantumType> item) => _images.Contains(item);
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickImage<QuantumType> Flatten() => Merge(LayerMethod.Flatten);
    ...
    public IEnumerator<IMagickImage<QuantumType>> GetEnumerator() => _images.GetEnumerator();
    ...
    public int IndexOf(IMagickImage<QuantumType> item) => _images.IndexOf(item);
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public void Map(IMagickImage<QuantumType> image) => Map(image, new QuantizeSettings());
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickImage<QuantumType> Merge() => Merge(LayerMethod.Merge);
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickImage<QuantumType> Mosaic() => Merge(LayerMethod.Mosaic);
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickErrorInfo? Quantize() => Quantize(new QuantizeSettings());
    ...
    public bool Remove(IMagickImage<QuantumType> item) => _images.Remove(item);
    ...
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickImage<QuantumType> SmushHorizontal(int offset) => Smush(offset, false);
    ...
    /// <returns>The resulting image of the smush operation.</returns>
    /// <exception cref="MagickException">Thrown when an error is raised by ImageMagick.</exception>
    public IMagickImage<QuantumType> SmushVertical(int offset) => Smush(offset, true);
        ...
        foreach (var image in _images)
        {
            if (ReferenceEquals(image, item))
                throw new InvalidOperationException(
                    "Not allowed to add the same image to the collection."
                );
        }
    }
    ...
    private MagickSettings GetSettings() => MagickImage.GetSettings(_images[0]);
    ...
    private void ThrowIfCountLowerThan(int count)
    {
        if (_images.Count < count)
            throw new InvalidOperationException(
                "Operation requires at least " + count + " images."
            );
    }
    ...
    )
    {
        if (
            _images.Count > 1
            && format.HasValue
            && MagickFormatInfo.Create(format.Value)?.SupportsMultipleFrames == false
        )
        {
            ...
            for (var i = 0; i < _images.Count; i++)
            {
                ...
                await _images[i]
                    .WriteAsync(newFileName, format.Value, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
        else
        {
            try
            {
                ...
                await FileHelper
                    .WriteAllBytesAsync(fileName, bytes, cancellationToken)
                    .ConfigureAwait(false);
            }
            finally
            {
