...
internal class ExifTagDescriptionGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(
            context => context.AddAttributeSource<ExifTagDescriptionAttribute>()
        );
        context.RegisterAttributeCodeGenerator<
            ExifTagDescriptionAttribute,
            IEnumerable<ExifTagDescriptionInfo>
        >(CreateInfo, GenerateCode);
    }

    private void GenerateCode(
        SourceProductionContext context,
        IEnumerable<ExifTagDescriptionInfo> descriptionInfos
    )
    {
        var codeBuilder = new CodeBuilder();
        codeBuilder.AppendLine("#nullable enable");
        codeBuilder.AppendLine();
        codeBuilder.AppendLine("using System.Runtime;");
        codeBuilder.AppendLine();
        ...
        codeBuilder.Indent++;
        codeBuilder.AppendLine(
            "public static Dictionary<ExifTagValue, Dictionary<object, string>> ForExifTagValue { get; } = new()"
        );
        codeBuilder.AppendLine("{");
        codeBuilder.Indent++;
        ...
        context.AddSource(
            "ExifTagDescriptions.g.cs",
            SourceText.From(codeBuilder.ToString(), Encoding.UTF8)
        );
    }

    private IEnumerable<ExifTagDescriptionInfo> CreateInfo(
