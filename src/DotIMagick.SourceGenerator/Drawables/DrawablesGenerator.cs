...
internal class DrawablesGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(
            context => context.AddAttributeSource<DrawablesAttribute>()
        );
        context.RegisterAttributeCodeGenerator<
            DrawablesAttribute,
            (bool, ImmutableArray<INamedTypeSymbol>)
        >(CheckForInterface, GenerateCode);
    }

    private static (
        bool GenerateInterface,
        ImmutableArray<INamedTypeSymbol> Interfaces
    ) CheckForInterface(GeneratorAttributeSyntaxContext context)
    {
        ...
        var interfaces = assembly.GlobalNamespace
            .GetNamespaceMembers()
    ...
    private static void AppendMethods(
        ...
        string name,
        ImmutableArray<PropertySymbolInfo> properties,
        bool generateInterface
    )
    {
        var isEnableProperty = properties.SingleOrDefault(
            property => property.ParameterName == "isEnabled"
        );
        ...
        var readOnlyListProperty = properties.SingleOrDefault(
            property =>
    ...
    private static void AppendEnableDisabledMethods(
        CodeBuilder codeBuilder,
        string name,
        PropertySymbolInfo property,
        bool generateInterface
    )
    {
    ...
    private static void AppendEnableDisabledMethod(
        CodeBuilder codeBuilder,
        string name,
        string type,
        string comment,
    ...
    )
    {
        ...
        codeBuilder.AppendReturnsComment(
            $"The <see cref=\"IDrawables{{{(generateInterface ? "T" : null)}QuantumType}}\" /> instance."
        );
    ...
    private static void AppendReadOnlyListMethod(
        CodeBuilder codeBuilder,
        INamedTypeSymbol type,
        string name,
        PropertySymbolInfo property,
    ...
    private static void AppendReadOnlyListImplementation(
        CodeBuilder codeBuilder,
        string name,
        PropertySymbolInfo property
    )
    ...
    private static void AppendDefaultMethod(
        CodeBuilder codeBuilder,
        INamedTypeSymbol type,
        string name,
        ImmutableArray<PropertySymbolInfo> properties,
    ...
    private void GenerateCode(
        ...
        (bool GenerateInterface, ImmutableArray<INamedTypeSymbol> Interfaces) info
    )
    {
        var allProperties = new Dictionary<INamedTypeSymbol, ImmutableArray<PropertySymbolInfo>>(
        ...
        codeBuilder.AppendLine(
            info.GenerateInterface ? "interface IDrawables<TQuantumType>" : "class Drawables"
        );
        codeBuilder.AppendLine("{");
        codeBuilder.Indent++;
