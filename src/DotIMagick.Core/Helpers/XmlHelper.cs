...
{
    public static XmlDocument CreateDocument() => new XmlDocument { XmlResolver = null, };

    public static XmlElement CreateElement(XmlNode node, string name, XmlDocument? doc)
    {
        doc = node.GetType() == typeof(XmlDocument) ? (XmlDocument)node : node.OwnerDocument;
