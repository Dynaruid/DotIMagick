...
public partial class ImageProfile : IImageProfile
{
    ...
    public override bool Equals(object? obj) => Equals(obj as IImageProfile);
...
#if NETSTANDARD2_1
    [Obsolete(
        $"This property will be removed in the next major release, use {nameof(ToByteArray)} or {nameof(ToReadOnlySpan)} instead."
    )]
#else
    [Obsolete(
        $"This property will be removed in the next major release, use {nameof(ToByteArray)} instead."
    )]
#endif
    ...
    /// </summary>
    /// <returns>A <see cref="byte"/> array.</returns>
    protected byte[]? GetDataProtected() => _data;

    /// <summary>
    /// Sets the data of the profile.
    /// </summary>
    /// <param name="data">The new data of the profile.</param>
    protected void SetData(byte[]? data) => _data = data;
